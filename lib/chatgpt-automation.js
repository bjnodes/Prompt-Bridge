...
class ChatGPTAutomation {
  ...
  async isOnChatPage() {
    ...
    try {
      ...
      return currentUrl.startsWith(this.chatUrl);
    } catch {
      return false;
    }
  }

  async navigateToChatIfNeeded() {
  ...
  async start() {
    ...
    try {
      await this.navigateToChatIfNeeded();
    } catch {
      this.resetRuntimeState();
      await this.launchContext();
      await this.navigateToChatIfNeeded();
  ...
  async waitForResponse(previousCount) {
    ...
    for (let attempt = 0; attempt < 240; attempt += 1) {
      ...
      if (currentText) {
        bestCandidate = currentText;
      }

      const looksStructured =
        currentText &&
        currentText.includes('"title"') &&
        currentText.includes('"prompt"') &&
        currentText.includes('"notes"');

      if ((currentCount > previousCount || looksStructured) && currentText) {
        ...
        if ((!stopVisible && stableRepeats >= 2) || stableRepeats >= 4) {
          return currentText;
        }
      }
    }

    const fallback = (await this.extractLastAssistantText()) || bestCandidate;
    if (!fallback) {
  ...
  async clickSendIfPossible() {
    const sendSelectors = [
      ...
      "button[aria-label*='send']:visible",
      "button[aria-label*='메시지 보내기']:visible",
      "button[aria-label*='전송']:visible",
  ...
  async submitComposerFormIfPossible(composer) {
    try {
      const submitted = await composer.evaluate((node) => {
  ...
  async sendPrompt(promptText, options = {}) {
    ...
    let submitted = await this.submitComposerFormIfPossible(composer);
    if (!submitted) {
      submitted = await this.clickSendIfPossible();
    }
    if (!submitted) {
      await composer.focus().catch(() => null);
      await this.page.keyboard.press("Control+Enter").catch(() => null);
      await this.page.waitForTimeout(250);
    }
    if (!submitted) {
      await composer.focus().catch(() => null);
